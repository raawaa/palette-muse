...
class ColorNamerTest {
    ...
    fun returns_two_words() {
        val parts = namer.nameColor("#FF0000").split(" ")
        assertEquals("Expected \"<mood> <base>\"", 2, parts.size)
    }

    @Test
    fun red_hue_classified() {
        assertTrue(namer.nameColor("#FF0000").endsWith("Red"))
    }

    @Test
    fun orange_hue_classified() {
        assertTrue(namer.nameColor("#FF7F00").endsWith("Orange"))
    }

    @Test
    fun yellow_hue_classified() {
        assertTrue(namer.nameColor("#FFD700").endsWith("Yellow"))
    }

    @Test
    fun green_hue_classified() {
        assertTrue(namer.nameColor("#00FF00").endsWith("Green"))
    }

    @Test
    fun blue_hue_classified() {
        assertTrue(namer.nameColor("#0000FF").endsWith("Blue"))
    }

    @Test
    fun purple_hue_classified() {
        assertTrue(namer.nameColor("#800080").endsWith("Purple"))
    }
    ...
    fun red_orange_boundary_first_range_wins() {
        ...
        assertTrue(namer.nameColor("#FF3F00").endsWith("Red"))
    }
    ...
    fun purple_red_boundary_first_range_wins() {
        ...
        assertTrue(namer.nameColor("#FF0040").endsWith("Purple"))
    }
    ...
    fun lowValue_only_uses_baseName() {
        ...
        val parts = namer.nameColor("#3C0A0A").split(" ")
        assertNotEquals("Red", parts[1])
    }

    @Test
    fun lowSaturation_uses_baseName() {
        val parts = namer.nameColor("#808080").split(" ")
        assertNotEquals("Red", parts[1])
    }
